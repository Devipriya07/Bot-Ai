...
export default function Home() {
    ...
    const generateResponse = (UserInput) => {
        ...
        const response = data.find(item => UserInput.toLowerCase() === item.question.toLowerCase());
        let answer = "Sorry, Did not understand your query!"
        if (response !== undefined) {
            answer = response.response
        }
        setChat( prev => ([...prev, { type: 'Human',
                                      text: UserInput,
                                      time: new Date(),
                                      id: chatId
                                    },
    ...
    <Box>
        <Stack height={'100vh'} justifyContent={'space-between'}
            ...
            {chat.length > 0 && (
                <Stack height={1} flexGrow={0} ref={listRef}
                       p={{ xs: 2, md: 3 }}
                       spacing={{ xs: 2, md: 3 }}
                       sx={{
                        overflowY: 'auto',
                        '&::-webkit-scrollbar': { // pseudo-elements to customize scrollbar
                            width: '10px',
                        },
